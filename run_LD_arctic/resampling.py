...
### test input
script="resampling.py"
expdir="/gpfs/work/IscrB_INCIPIT/gzappa/PLASIM-large-dev/run_large_dev_test"
expname="run_large_dev_test"
ntrajs=int(4)
block=int(1)
varname="ts"
k=float(10)
dt=float(0.01389)
maskname="most_t42_lsmask.nc"
gpareaname="most_t42_gparea.nc"
### end test input
...
# ### definisce mask scandinavia ###
# mask_Scandinavia=np.zeros(mask.shape)
# mask_Scandinavia[mask>0]=1
# mask_Scandinavia[LAT>72]=0
# mask_Scandinavia[LAT<55]=0
# mask_Scandinavia[LON>40]=0
# mask_Scandinavia[LON<5]=0
# mask_Scandinavia[np.logical_and(LON>39,LAT<62)]=0
# mask_Scandinavia[np.logical_and(LON>33,LAT<59)]=0
# mask=mask_Scandinavia*gparea 
# areamask=np.sum(mask,axis=(0,1)) 

### definische mask Arctic
mask_Scandinavia=np.zeros(mask.shape)
mask_Scandinavia[mask>0]=1
mask_Scandinavia[LAT>72]=0
mask_Scandinavia[LAT<55]=0
...
for traj in range(ntrajs):
    ...
    for t in range(obsts.shape[1]):
        obsts[traj,t]=np.sum(np.squeeze(obs[t,:,:])*mask,axis=(0,1))/areamask
    obsmean[traj]=np.sum(obsts[traj,:])*dt # vettore che per ogni traiettoria ha valore media osservabile

#print(obsmean)
#print(np.mean(obsmean))
...
killedlabel=np.empty(ntrajs)  # array, 0 se traiettoria  sopravvisuta, 1 se ammazzata
print("killedlabel ", killedlabel)
for traj in range(ntrajs):
    if nc[traj]>0:
        killedlabel[traj]=0
    else:
...
# SE POSIZIONE 1 C'e' 3, ALLA PROSSIMA SIMULAZIONE TRAIETTORIA 1 PRENDE RESTART 3
