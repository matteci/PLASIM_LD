...
resamplingname='resampling_Amoc_abs.py'   # file che fa il resampling
ntrajs=10
k=KKKK
NMonths=0  # length resampling block
NDays=NNNN # length resampling block
LYear=360  # 
startID=YYYY # 0BCD (B: ocean state, C: atmospheric state, D: repeat)
...
#
# Directory names
homedir='/work/PLASIM/PLASIM-master/'
...
#      srun --mpi=pmi2 -K1 --resv-ports --exclusive --nodes=1 --ntasks=${nparallel} --mem=${mem} ${modelname} & # fai partire modello
  ...
  for a in `ls ${expdir}/data/${blocklabel}/*.${blocknumber}`; do 
      ...
      srv2nc -m -p ${a} ${expdir}/data/${blocklabel}/netcdf/${fname}.nc &
  done
  wait
  ## ## ## ## ## ##
