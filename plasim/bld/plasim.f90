...
!     ============================
!     SUBROUTINE NOISERESTART
!     added on 12/01/2019 by
!     francesco.ragone@ens-lyon.fr
!     ============================
...
!     if kickrestart is set to 1 or 2
!     adds white noise perturbation to ln(surface pressure)
!     balanced initial state at t=0.
!     for kickrestart=2, the white noise pertubation is
!     symmetric to the equator
!     epsrestart sets magnitude of the noise

      itp1 = NTP1 ! Suppress compiler warnings for T1
      if (itp1 <= 2 .and. kickrestart > 2) kickrestart = 0 ! for T1
      zscale=epsrestart/sqrt(2.0)

      write(nud,'(/," *****************************************")')
      if (kickrestart == 1) then
         jsp1=2*NTP1+1
         do jsp=jsp1,NRSP
            call random_number(zrand)
            if (mrpid > 0) zrand = zrand + mrpid * 0.01
            sp(jsp)=sp(jsp)+zscale*(zrand-0.5)
         enddo
         write(nud,'(" *     White noise added (KICKRESTART = 1)      *")')
      elseif (kickrestart == 2) then
         jr=2*NTP1-1
         do jm=1,NTRU
            do jn=jm,NTRU
               jr=jr+2
            ...
            enddo
         enddo
         write(nud,'(" * Symmetric white noise added (KICKRESTART=2) *")')
      elseif (kickrestart == 3) then
         sp(2*itp1+3) = zscale
         sp(2*itp1+4) = zscale * 0.5
         write(nud,'(" *  Mode sp(1,1) disturbed (KICKRESTART = 3)    *")')
      endif
      write(nud,'(" *****************************************")')
      return
      end
